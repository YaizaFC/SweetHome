...
class User implements UserInterface{
    ...
    protected $password_min_score = 1;
    ...
    */
	...
	/**
	* Returns if user is an owner or not.
	...
	**/
	...
	/*
	* Returns the user's home (if he is owner and has one published)
	* @return integer|false Number of home's ID if true, false if not.
	*/
	public function getOwnerHomeID(){
		if(!$this->isOwner()){return false;}
		$getHome = $this->db->select("casas", array('casa_owner' => $this->userID), array('casa_id'));
		return $getHome['casa_id'];
	}
}